...
// older compilers might need this
#include <cstdint>
#include <cstdlib>
#include <cstring>
...
struct Vertex {
  ...
  static VkVertexInputBindingDescription getBindingDescription() {
    ...
    // This determines how you move to the next data, either move after each
...
private:
  ...
  VkDebugUtilsMessengerEXT debugMessenger; // tell Vulkan about debug callback
  ...
  std::vector<VkImageView> swapChainImageViews; // store the images views in.
  std::vector<VkFramebuffer> swapChainFramebuffers; // hold framebuffers for
                                                    // images in swapchain
  ...
  VkDescriptorSetLayout
      descriptorSetLayout; // describes the type of descriptors that can be
                           // bound,contains all descriptor bindings.
  VkDescriptorPool descriptorPool; // Descriptor sets are created from a pool.
  ...
  VkPipelineLayout
      pipelineLayout; // uniform values for shaders that can be
                      // changed at drawing time. The sequence of descriptor set
                      // layouts that can be used by a pipeline. Determine the
  ...
  std::vector<VkCommandBuffer>
      commandBuffers; // Each command buffer records commands that will be
                      // submitted to device queue for execution.
  ...
  VkSampleCountFlagBits msaaSamples =
      VK_SAMPLE_COUNT_1_BIT; // for MSAA, will later store max supported
                             // samples. Default is 1 (No multisampling)
  ...
  std::vector<void *> uniformBuffersMapped; // CPU memory mapped to GPU
  ...
  std::uint32_t mipLevels; // controls Texture LoD settings
  ...
  // Image stores the desired number of samples per pixel.
  VkImage colorImage;
  ...
  void createInstance() {
    ...
    // Everything is ready to create an instance, Create functions returns
    // VkResult, check for failure
  ...
  VkSampleCountFlagBits getMaxUsableSampleCount() {
    ...
    // Extracts the maximum number of samples associated with out physical
    // device
    vkGetPhysicalDeviceProperties(physicalDevice, &physicalDeviceProperties);

    // We are using a depth buffer, so we have to take into account the sample
    // count for both color and depth. The highest supported by both will be the
    // highest we can support
  ...
  bool checkDeviceExtensionSupport(VkPhysicalDevice device) {
    ...
    vkEnumerateDeviceExtensionProperties(
        ...
        nullptr); // queries number and fills extensionCount
    ...
    return requiredExtensions.empty(); // all required extensions are present
  }
  ...
  void createLogicalDevice() {
    ...
    QueueFamilyIndices indices =
        findQueueFamilies(physicalDevice); // current physical device
    ...
    // specify sample shading feature for the device
    ...
    if (enableValidationLayers) {
      ...
      // Validation). Include for compatiblity reasons
      createInfo.enabledLayerCount =
          static_cast<std::uint32_t>(validationLayers.size());
      createInfo.ppEnabledLayerNames = validationLayers.data();
    } else {
    ...
    // once(both handles will have the same value).
  ...
  SwapChainSupportDetails querySwapChainSupport(VkPhysicalDevice device) {
    ...
    // struct.
    // Takes in specified VkPhysicalDevice and VkSurfaceKHR window surface
    // into account.
  ...
  VkSurfaceFormatKHR chooseSwapSurfaceFormat(
    ...
    // VK_FORMAT_B8G8R8A8_SRGB stores B G R A channels in 8Bit (32Bits per
    // pixel). colorSpace indicates if SRGB colour space is supported using the
  ...
  VkExtent2D chooseSwapExtent(const VkSurfaceCapabilitiesKHR &capabilities) {
    // Use currentExtent member of VkSurfaceCapabilitiesKHR, unless max.
    ...
    if (capabilities.currentExtent.width !=
      ...
      // max supported by the implementation.
      // returns passed value unless it is higher or lower than the bounds, then
      // it returns the closest bound.
      actualExtent.width =
  ...
  void createSwapChain() {
    ...
    createInfo.surface = surface; // Surface attached to instance
    ...
    // operation like post processing.
    // In that case use VK_IMAGE_USAGE_TRANSFER_DST_BIT and then use a memory op
    ...
    createInfo.clipped =
        VK_TRUE; // TRUE means we don't care about the color of obscured pixels.
                 // Use TRUE unless it is really needed
  ...
  void createRenderPass() {
    ...
    // For DONT_CARE, existing contents are undefined, CLEAR clears the value
    // to a constant. LOAD preserves existing contents.
    // We want to use the clear operation to clear the framebuffer to black
    // before drawing a new frame.
    colorAttachment.loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
    // Rendered content will be stored in memory and can be read later.
    // We are interested in seeing the rendered image on the screen
    ...
    //
    // But we are using multisampled images, which cannot be presented
    // directly. They would be resolved to a regular image. This requirement
    // does not apply to the depth buffer as it is not presented at any point.
    // We would need to add an extra color attachment called the resolve
    // attachment.
    ...
    // Extra color attachment
    // Will be used to instruct render pass to resolve multisampled color image
    // to regular attachment, it would need a new attachment reference.
    ...
    // Not presenting yet, ImGui UI will be overlayed over our image.
    colorAttachmentResolve.finalLayout =
        VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;

    // Attachment reference that will point to color buffer which will serve as
    // the resolve target.
    ...
    // subpass
    // Added AttachmentReferences to the first (and only) subpass
    // a subpass can only use a single depth(+stencil) attachment but can use
    // multiple color attachments, . Doing depth tests on multiple buffers
    // doesn't make sense
    ...
    // points to the resolve attachment reference, this is enough to let the
    // render pass define a multisample resolve operation which will let us
    ...
    // This refers to the implicit subpass before or after the render pass
    ...
    // so wait on the color attachment output stage itself. Depth image is first
  ...
  void createDescriptorSetLayout() {
    ...
    // Can be used to specify a transformation for each bone in a skeleton for
  ...
  void createGraphicsPipeline() {
    ...
    auto vertShaderCode = readFile(
        "resources/shaders/vert.spv"); // path relative to the executable
    ...
    // Min fraction for sample shading: closer to 1 is smoother
    ...
    // We are sticking to the convention of lower depth = closer, so the depth
    // of new fragments should be less to appear in front.
    ...
    // A depth stencil state must always be specified if the render pass
    ...
    // Then the render pass and index of the subpass where graphics pipeline is
  ...
  VkShaderModule createShaderModule(const std::vector<char> &code) {
    // specify pointer to the buffer with the bytecode and length
  ...
  void createFramebuffers() {
    ...
    for (size_t i = 0; i < swapChainImageViews.size(); i++) {
      // bind the depth image to the depth attachment as a second attachment
      ...
      framebufferInfo.height = swapChainExtent.height;
      framebufferInfo.layers = 1;
      imgui_attachment[0] = swapChainImageViews[i];
      if (vkCreateFramebuffer(device, &framebufferInfo, nullptr,
  ...
  void createCommandPool() {
    ...
    // ImGui Command pool
    if (vkCreateCommandPool(device, &poolInfo, nullptr, &imGuiCommandPool) !=
        VK_SUCCESS) {
      throw std::runtime_error("Could not create ImGui graphics command pool");
    }
  }

  // Abstracts image creation, handles image object creation and memory
  // allocation.
  void createImage(std::uint32_t width, std::uint32_t height,
                   std::uint32_t mipLevels, VkSampleCountFlagBits numSamples,
    ...
    // you can stick to VK_SAMPLE_COUNT_1_BIT otherwise
  ...
  VkFormat findSupportedFormat(const std::vector<VkFormat> &candidates,
    ...
    for (VkFormat format : candidates) {
      // contains 3 fields, only linearTilingFeatures and optimalTilingFeatures
      // are relevant here
      VkFormatProperties props;
      vkGetPhysicalDeviceFormatProperties(physicalDevice, format, &props);

      // The one we check depends on the tiling parameter of the function
  ...
  void createColorResources() {
    ...
    // We are passing in our max number of samples as the number of samples.
    // We're also using only one mip level, since this is enforced by the Vulkan
    // specification in case of images with more than one sample per pixel.
    // Also, this color buffer doesn't need mipmaps since it's not going to be
    // used as a texture:
    createImage(swapChainExtent.width, swapChainExtent.height, 1, msaaSamples,
  ...
  void createCommandBuffers() {
    ...
    imGuiCommandBuffers.resize(
        swapChainImageViews.size()); // or MAX_FRAMES_IN_FLIGHT?
    ...
    vkAllocateCommandBuffers(device, &allocInfo, imGuiCommandBuffers.data());
  }
  ...
  void createSyncObjects() {
    ...
    // This is a work around so that for the first time we call vkWaitForFences
  ...
  // Draw call
  // Added ImGui code and clear color parameter for ImGui UI
  void drawFrame(ImVec4 clear_color) {
    ...
    // Recording ImGui Command Buffer
    {
      ...
      // Render pass
      {
        ...
        vkCmdBeginRenderPass(imGuiCommandBuffers[currentFrame], &info,
                             VK_SUBPASS_CONTENTS_INLINE);
      }
      ...
      {
        err = vkEndCommandBuffer(imGuiCommandBuffers[currentFrame]);
        check_vk_result(err);
      }
    }

    // Submitting commands to the GPU, added ImGui command buffers
    ...
    // Take signalSemaphore which will be signaled and wait on them.
    ...
    // Not necessary if you're only using a single swap chain (just use the
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR || result == VK_SUBOPTIMAL_KHR ||
      ...
      // consistent state otherwise a signaled semaphore may never be properly
      // waited upon
  ...
  static void framebufferResizeCallback(GLFWwindow *window, int width,
    ...
    auto app = reinterpret_cast<ImguiVulkanGlfwApplication *>(
  ...
  void initWindow() {
    ...
    // Actual window
    window = glfwCreateWindow(WIDTH, HEIGHT, "Dear ImGui GLFW+Vulkan example",
                              nullptr,
  ...
  void cleanup() {
    ...
    // vertex buffer should be available for use for rendering commands until
    // the end of the program. No dependance on swap chain.
    ...
    vkFreeCommandBuffers(device, imGuiCommandPool,
                         static_cast<uint32_t>(imGuiCommandBuffers.size()),
